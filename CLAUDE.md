...
- **libsql ↔ sqld WriteProxy header mismatch — worked around in the proxy.** sqld 0.24.32's WriteProxy gRPC service expects `x-proxy-authorization` to be a JSON-serialized `libsql_server::auth::Authenticated` enum (its intra-cluster auth handoff format). libsql clients (0.6 through 0.9.30) only send `x-authorization` and never set the proxy header, so embedded-replica writes failed with `"x-proxy-authorization not set"` or a cryptic h2 `"Invalid header bit X"` panic. The shoebox-server proxy now injects `x-proxy-authorization: "FullAccess"` on every outgoing gRPC request — safe because the proxy already authenticated the caller via mTLS and sqld is loopback-only inside the same process tree. Upstream tracking: tursodatabase/go-libsql#42 (open since 2024-09); unlikely to be fixed since Turso deprecated this codepath in favour of Turso Sync. See `crates/shoebox-server/src/proxy.rs::forward_http`.
